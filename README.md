...
1. `头文件共有继承Nodelet，私有成员包含一个类的的智能指针； 同时重写onInit（）函数`
...
```
2. `cpp 文件中实现onInit()函数， 同时将新的nodelet添加到pluginlib`
```
namespace msckf_vio {
...
4. `在包下新建一个nodelets.xml 文件， 在里面指定nodelet的lib路径以及nodelet类的名字`
    ```

    ```
5. ` 在package.xml中增加export项，将上面的配置文件添加进去` 
   ```
     <export>
