...
private:
   ...
   *    特征检测和追踪的配置参数，将特征点分配到棋盘网格中，并设定每个栅个特征点的提取的数目，
   *    还有特征匹配的阈值
   */
   ...
   *    特征点的数据包含id号,强度，还有生存时间，在左右两个相机的单位成像平面的坐标
   */
   ...
   *    加载IMU和相机矫正的参数，包含IMU和相机位姿变换矩阵，还有相机的的的参数
   */
  bool loadParameters();
