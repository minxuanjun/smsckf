...
struct Feature {
   ...
   *    poses is sufficient.
   *    通过检查观察到特征点起始相机位姿和最终相机位姿的translation和特征点在起
   *    使相机位姿的垂直方向的分量是否大于一定的阈值,如果足够大,则返回true
   */
  ...
  // s(key)-image_ctate_idoordinates(value) manner.
...
// 非常easy
// 三角测量恢复深度
// s2*x2 = s1R_1_2*x1 + t_1_2
//s2*[x2]^x2 = s1 * [x2]^ * R_1_2*x1 + [x_2]^ * t_1_2
// 0 = s1*[x2]^*R_1_2*x1 + [x_2]^ * t_1_2
...
bool Feature::initializePosition(
  ...
  for (auto& m : observations) {
    ...
    // Add the measurement.
    // m 的数据类型是map [cameraID, meansurement [x1,y1, x1, y2]]
