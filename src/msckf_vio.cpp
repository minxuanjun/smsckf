...
bool MsckfVio::loadParameters()
{
    ...
    state_server.imu_state.t_cam0_imu = T_cam0_imu.translation();// cam0 在 imu 坐标系下的位置
...
bool MsckfVio::createRosIO()
{
    ...
    feature_sub = nh.subscribe("features", 1000,
    ...
    const sensor_msgs::ImuConstPtr &msg)
{
    ...
    if (!is_gravity_set) {
        ...
        is_gravity_set = true;
        ROS_WARN("IMU initialize completed");
    }
...
void MsckfVio::initializeGravityAndBias()
{
    ...
    // 世界坐标系（惯性坐标系）,当载具刚开始静止的时候, 陀螺仪测得的加速度为
    // am = C_g_imu*(-g) , 为了计算C_g_imu, 我们可以计算 am 和 -g 之间的rotatio matrix,
    // 还有一个需要注意的是, 世界坐标系的z轴朝上
    ...
    const CameraMeasurementConstPtr &msg)
{
    ...
    ros::Time start_time = ros::Time::now();
    {
       ...
       // con.wait(lock, [&](){ return !imu_msg_buffer.empty();});
        batchImuProcessing(msg->header.stamp.toSec());
    }
    cout << "batchImuProcessing" << std::endl;
    double imu_processing_time = (
        ros::Time::now() - start_time).toSec();
    // Augment the state vector.
    // 状态增强：将新的相机的状态的协方差矩阵加入到协方差矩阵中,
    // Pnew_cam_imu= J_new_cam_imu * P_imu_imu
    // Pnew_cam_cam= J_new_cam_imu * P_imu_cam
    // Pnew_cam_new_cam = J_new_cam_imu * P_imu_imu * J_new_cam_new_cam.T
    start_time = ros::Time::now();
    stateAugmentation(msg->header.stamp.toSec());
    double state_augmentation_time = (
        ros::Time::now() - start_time).toSec();
    cout << "stateAugmentation" << std::endl;
    // Add new observations for existing features or new
    // features in the map server.
    // 增加以前特征点新的观测值或者增加新的特征点到map_server中
    start_time = ros::Time::now();
...
void MsckfVio::batchImuProcessing(const double &time_bound)
{
    ...
    ROS_INFO_STREAM("batchImuProcessing: "<< imu_msg_buffer.size());
    if(imu_msg_buffer.front().header.stamp.toSec() > time_bound)
    {
        imu_msg_buffer.clear();
        return;
    }
                            ...
                            const Vector3d &m_acc)
{
    ...
    // 参考 Observability-constrained Vision-aided Inertial Navigation 论文
    // 见公式21 22 23
    // A[0:3, 0:3] = Rt_t+1 = R_w_imu_t+1* (R_w_imu_t).T
    ...
    //A1*u*s = A1- A1*R_k_k+1 * g*gT/(||g||2)
    // w1 * s  = (v_k - v_k+1)^R_k_k+1 * g*gT /||g||2
...
//四元数按照匀角速度模型进行积分,速度和位置按照4阶Runge-Kutta 进行积分
                               ...
                               const Vector3d &acc)
{
    ...
    // qL = cos(|w|*deltaT/2)*I_4x4 + 1/(|w|)*sin(|w|deltaT/2)*Omega
    // 如果 |w|很小
    // qL = I_4x4 + deltaT/2*Omega
    ...
    // ds_dt_transpose dR_dt2_transpose R_imu_w
...
//     J.block<3, 3>(3, 18) = Matrix3d::Identity();
    ...
    // P_new_cam_new_cam = J_cam*P_old_cam_old_cam*J_cam.T
...
void MsckfVio::addFeatureObservations(
    const CameraMeasurementConstPtr &msg)
{
    // imu_state_id 表示的就是相机pose的id
    StateIDType state_id = state_server.imu_state.id;
    int curr_feature_num = map_server.size();  // 获取特征点的数目
    ...
    for (const auto &feature : msg->features) {
        // 如果在map_server中未找到特征点的ID,表示该特征点是新的特征点
    ...
    MatrixXd &H_x, VectorXd &r)
{
    ...
    // 检测feature真的被哪些相机观测到， 然后将合法的相机id存放到vector中
    // 实际上，我感觉这个步骤是多余的
    ...
    // of H_fj.
    // ||A.T * U*D*V.T||2  = ||V*D.T*U.T*A|| = ||D*U.T*A||=0 
    // y = U.T*A 
    // y的前三列为零，上式才会等于0
    // A = U*y, y的前三列为零
    ...
    const MatrixXd &H, const VectorXd &r)
{
    ...
    MatrixXd H_thin;
    VectorXd r_thin;
    // 如果观测的的维度大于状态变量的维度，采用QR分解可以提取主要的信息，
    //忽略次要的信息，降低计算所用的时间
    if (H.rows() > H.cols()) {
...
void MsckfVio::removeLostFeatures()
{
    ...
    for (auto iter = map_server.begin();
        ...
        if (feature.observations.find(state_server.imu_state.id) !=
            feature.observations.end())
            continue;
        // 如果以前的特征在当前帧中track到,那么我们需要判断特征点被观测到的次数,
        // 如果追踪的的次数小于3,那么该特征点是非法的特征点
        ...
        // has not been.
        // 如果特征点没有被初始化,那么三角化，若果三角化后的特征点的深度值是负深度，
        //这个点也不在也不在观测方程内
        ...
        // 雅克比矩阵H_f真实的维度是[4n,3],因为H_f 是列满秩， 所以H_f的左零空间是维度为4n-3
...
// 采用关键帧的思想，剔除的多余的相机的状态，最新的相机的状态不能剔除掉，
// 只能剔除t-3, t-2， 或者0,1,首先判断t-3,t-2能否剔除掉，如果能剔除掉，0,1保留
...
void MsckfVio::pruneCamStateBuffer()
{
    ...
    ROS_INFO_STREAM("rm_cam_state_ids" << rm_cam_state_ids.size());
