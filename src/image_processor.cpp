...
{
    // 创建相机测量值（h特征点的左右成像坐标）和追踪状态（追踪前、追踪后、匹配后和RANSAC一直后特征点的数目）的的Publisher
    ...
    cam0_img_sub.subscribe(nh, "cam0_image", 1000);
    cam1_img_sub.subscribe(nh, "cam1_image", 1000);
    ...
    vector<cv::Point2f> &compensated_pts)
{
    ...
    cv::Matx33f K(
        ...
        0.0, 0.0, 1.0);
    // 忽略平移运动，只考虑旋转运动，来计算单应矩阵
    // p_c = K*R_p_c *K_inv * p_p
...
void ImageProcessor::trackFeatures()
{
    ...
    // 将特征点的id, lifetime, 在左右相机中的成像坐标解算出来
    ...
    // as untracked.
    // 将追踪到的特征点在当前帧图像外围的特征点标记为外点，即追踪失败
    ...
    //
    // 1) Stereo matching between current images of cam0 and cam1, 使用了点到极线的距离作为了判断条件,
    ...
    // twoPointRansac 实际上还是使用的点到极线的距离作为判定准则
    ...
    vector<unsigned char> &inlier_markers)
{
    ...
    if (cam1_points.size() == 0) {
        ...
        //将cam0中的特征点旋转到第二张图片中,用来初始化对饮的特征点在cam1中的像素坐标
    ...
    // 计算基础矩阵 E = p1.T*t_cam0_cam1^*R_cma0_cam1*p0
    ...
    // norm_pixel_unit 表示每个像素代表实际距离多少米
    ...
    for (int i = 0; i < cam0_points_undistorted.size(); ++i) {
        ...
        // operator* 重载了矩阵和向量的乘法
        // error 计算的是点到极线的距离，如果点到极限的距离过大，表示这个特征点不合法,可能属于运动物体上的特征点
    ...
    Matx33f &cam0_R_p_c, Matx33f &cam1_R_p_c)
{
    ...
    // 注意opencv中的Rodrigues使用的是由theta_i_v 来计算R_v_i, 所以要计算R_i_v 只需要将R_v_i 转置即可,参考机器人学的状态估计p169
    ...
    vector<int> &inlier_markers)
{
    ...
    //TODO: 感觉pts1_undistorted 计算有问题， 应该是pt.x = pt_hc[0] / pt_hc[2], pt.y = pt_hc[1] / pt_hc[2]
    ...
    // be almost 0.
    // 在使用2-point RANSAC, 我们需要检查一下，运动是否退化,即两个坐标系之间没有平移,
    // 在这种情况下,2-point RANSAC 失效。
    // 2 point RANSAC 可以采用极坐标系或者笛卡尔坐标系进行求解，程序中实现的是争对的
    // 笛卡尔坐标系。
    // 设 t_p_c = [a1, a2, a3]
    // p2.T * [t_p_c]×I * p1 = 0
    // p2  = [x2 y2, 1], p1 = [x1, y1, 1]
    // 展开得到方程
    // a1*(y1-y2) -a2(x1-x2) + a3*(x1y2 - x1y1) = 0
    // 方程有三个参数, 其实应该只有两个参数,一个参数用来控制尺度
...
void ImageProcessor::drawFeaturesStereo()
{
    ...
    if (debug_stereo_pub.getNumSubscribers() > 0) {
        ...
        // Collect feature points in the previous frame.
        // prev_cam0_points、 prev_cam1_points 数据类型是map， 将特征点的id号映射为特征点的位置
